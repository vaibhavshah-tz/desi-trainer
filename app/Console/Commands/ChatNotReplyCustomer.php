...
class ChatNotReplyCustomer extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'chat:not-reply-customer';
     ...
     * @var string
     */
    protected $description = 'Not reply to staff after 10 min for chat for customer.';
        ...
        try {
            ...
            if ($chatRoom->count() > 0) {
                foreach ($chatRoom as $key => $value) {
                    ...
                    if (!empty($chatMessage) && $chatMessage->sender_type != config('constants.SENDER_TYPE.CUSTOMER') && $currentUtcDate->diffInMinutes($chatMessage->created_at) <= config("constants.CRON_TIME.CHAT_NOT_RPY_MINUTE")) {
                        $notificationData = [
                            'sender_id' => config('constants.ADMIN_ROLE.SUPER_ADMIN'),
                            'receiver_id' => $value->customer_id,
