...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            // }
            if (isset($item['submenu']) && self::isActiveVerMenuItem($item, \Route::currentRouteName())) {
                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
            } elseif (self::isActiveVerMenuItem($item, \Route::currentRouteName())) {
                $item_class .= ' menu-item-active';
            }
            ...
            if (isset($item['parent'])) {
                ...
                if (isset($item['page'])) {
                    // $url = url($item['page']);
                    if (request()->route('id')) {
                        $url = route($item['page'], ['id' => request()->route('id')]);
                    } else {
                        $url = route($item['page']);
                    }
                }
            ...
            if (isset($item['submenu'])) {
                ...
                foreach ($item['submenu'] as $submenu_item) {
                    if (isset($submenu_item['permissions'])) {
                        if (PermissionHelper::hasAccess($submenu_item['permissions'])) {
            ...
            foreach ($item as $each) {
                if (isset($each['permissions'])) {
                    if (PermissionHelper::hasAccess($each['permissions'])) {
    ...
    public static function renderHorMenu($item, $parent = null, $rec = 0)
    {
        ...
        if (isset($item['separator'])) {
            ...
            //     }
            // }
            if (isset($item['submenu']) && self::isActiveHorMenuItem($item, \Route::currentRouteName())) {
                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active

                if (@$item['submenu']['type'] == 'tabs') {
                    $item_class .= ' menu-item-active-tab ';
                }
            } elseif (self::isActiveHorMenuItem($item, \Route::currentRouteName())) {
                $item_class .= ' menu-item-active ';
            ...
            //     $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
            // }
            if (isset($item['submenu']['items']) && self::isActiveHorMenuItem($item['submenu'], \Route::currentRouteName())) {
                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
            }
                ...
                if (isset($item['submenu'])) {
                    if (in_array($item['submenu']['type'], array('classic', 'tabs'))) {
                        ...
                        foreach ($item['submenu']['columns'] as $column) {
                            ...
                            //     $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
                            // }
                            if (isset($column['items']) && self::isActiveVerMenuItem($column, \Route::currentRouteName())) {
                                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
                            }
    ...
    {
        $activePage = isset($item['page']) ? explode('.', $item['page']) : '';
        if (@$item['redirect'] === true) {
            return false;
        }
        ...
        if (isset($item['page']) && $activePage[0] != 'tickets' && Str::contains($page, $activePage[0])) {
            return true;
        } elseif (isset($item['page']) && $item['page'] == $page) {
            return true;
        }
