...
<div class="card card-custom">
    <div class="card-header">
        <div class="card-title">
            ...
            <h3 class="card-label">{{ __('Course Category Listing') }}</h3>
        </div>
        <div class="card-toolbar">
            <!--begin::Button-->
            <button type="button" id="modelAdd" class="btn btn-primary font-weight-bolder" data-toggle="modal" data-target="#exampleModal" data-toggle="tooltip" data-theme="dark" title="{{ __('Add course category') }}">
                <i class="la la-plus"></i>{{ __('Add course category') }}
            </button>
            <!--end::Button-->
        </div>
    </div>
    <div class="card-body">
        <!--begin: Datatable-->
        <table class="table table-separate table-head-custom table-checkable coursecat-tble" id="course_category_datatble">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Name</th>
...
@endslot
@slot('form')
{{ Form::open(['method' => 'POST', 'route' => 'course.category.store', 'id' => 'course-category-add','class' => 'form', 'enctype' => 'multipart/form-data' ]) }}
@include('courseCategory._form')
{{ Form::close() }}
@endslot
@endcomponent
...
<script src="{{ asset('js/pages/custome-validation/course-category.js') }}" type="text/javascript"></script>
...
<script>
    var KTDatatablesSearchOptionsColumnSearch = function() {
        ...
        var initTable1 = function() {
            // begin first table
            var table = $('#course_category_datatble').DataTable({
                responsive: true,
                // Pagination settings
			...
			<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
                ...
                columnDefs: [{
                        ...
                        render: function(data, type, full, meta) {
                            return '\<button type="button" id="modelEdit" data-id="' + data.id + '" class="btn btn-sm btn-clean btn-icon edit-icon" data-toggle="tooltip" data-theme="dark" title="Edit details">\
                                \<i class="la la-edit"></i>\
                            \</button>\
                            \<a href="/course-category/delete/' + data.id + '" class="btn btn-sm btn-clean btn-icon delete-record" data-toggle="tooltip" data-theme="dark" title="Delete">\
    ...
    jQuery(document).ready(function() {
        ...
        /** Set the model add title and value */
        $(document).on("click", "#modelAdd", function() {
            $("#exampleModalLabel").html("Add course category");
            $("#course-category-add")[0].reset();
            $("#id").val('');
        });

        /** Set the model edit title and value */
        $(document).on("click", "#modelEdit", function() {
            $.ajax({
                type: 'GET',
                url: "/course-category/edit/" + $(this).data("id"),
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}"
                },
                success: function(data) {
                    ...
                    $("#id").val(data.id);
                    $("#exampleModalLabel").html("Edit course category");
