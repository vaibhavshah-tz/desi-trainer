@if($notifications->count() > 0)
    @foreach($notifications as $key => $value)
        <div id="block-{{ $value->id }}" class="d-flex align-items-center {{ ($value->is_read) ? 'bg-light-success' : 'bg-light-danger read-notification' }} rounded p-5 mb-9" data-id="{{ $value->id }}">
            <!--begin::Icon-->
            <span id="spanIcon-{{ $value->id }}" class="svg-icon {{ ($value->is_read) ? 'svg-icon-success' : 'svg-icon-danger' }}  mr-5">
            ...
            <div class="d-flex flex-column flex-grow-1 mr-2">
                <a href="{{ CommonHelper::setRedirectLink($value) }}" class="font-weight-bold text-dark-75 text-hover-primary font-size-lg mb-1">{{ $value->title ?? '' }}</a>
                <span class="text-muted font-weight-bold">{{ $value->message ?? '' }}</span>
            </div>
            <!--end::Title-->
            <!--begin::Lable-->
    ...
    @else
        <div class="d-flex align-items-center bg-light-success rounded p-5 mb-9">
            ...
            <div class="d-flex flex-column flex-grow-1 mr-2">
                ...
                <span class="text-muted font-weight-bold">{{ __('Notification not found.') }}</span>
            </div>
            <!--end::Title-->
            <!--begin::Lable-->
            <!-- <span class="font-weight-bolder text-success py-1 font-size-lg">{{ $value->created_at ?? '' }}</span> -->
